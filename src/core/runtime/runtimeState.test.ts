...
describe('RuntimeState', () => {
  ...
  describe('tool call recording', () => {
    it('should record tool call with metadata', () => {
      state.recordToolCall('ReadFile', { path: '/test.ts' }, { success: true, data: 'content' }, 123, true);
    ...
    it('should record multiple tool calls', () => {
      state.recordToolCall('ReadFile', { path: '/a.ts' }, { success: true }, 100, true);
      state.recordToolCall('WriteFile', { path: '/b.ts' }, { success: true }, 200, true);
      state.recordToolCall('RunCommand', { cmd: 'ls' }, { success: false, error: 'failed' }, 50, false);
  ...
  describe('snapshot and restore', () => {
    ...
    it('should restore from snapshot', () => {
      ...
      // Modify state
      state.addMessage({ role: 'assistant', content: 'Modified', timestamp: 2000 });
      state.incrementIteration();
      ...
      expect(state.getWorkspace().modifiedFiles.has('/modified.ts')).toBe(false);
    });
    ...
    it('should create deep copies in snapshot', () => {
      ...
      state.addMessage({ role: 'assistant', content: 'After', timestamp: 2000 });
  ...
  describe('immutability', () => {
    it('should return readonly conversation snapshot', () => {
      ...
      // Add another message
      state.addMessage({ role: 'assistant', content: 'Response', timestamp: 2000 });
