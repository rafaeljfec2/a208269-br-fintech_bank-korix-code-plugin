...
describe('IterationGuard', () => {
  ...
  describe('duplicate tool detection', () => {
    ...
    it('should emit duplicate_tool_detected event', () => {
      ...
      const duplicateEvent = events.find(e => e.type === 'duplicate_tool_detected');
  ...
  describe('no-progress detection', () => {
    ...
    it('should emit loop_warning event on no-progress', () => {
      ...
      guard.recordProgress({ iteration: 1, modifiedFiles: 2, toolCallCount: 5, timestamp: Date.now() });
      guard.recordProgress({ iteration: 2, modifiedFiles: 2, toolCallCount: 6, timestamp: Date.now() });
      guard.recordProgress({ iteration: 3, modifiedFiles: 2, toolCallCount: 7, timestamp: Date.now() });
    ...
    it('should not detect no-progress with fewer than 3 markers', () => {
      guard.recordProgress({ iteration: 1, modifiedFiles: 1, toolCallCount: 1, timestamp: Date.now() });
      guard.recordProgress({ iteration: 2, modifiedFiles: 1, toolCallCount: 2, timestamp: Date.now() });
  ...
  describe('reset', () => {
    it('should reset all counters and progress markers', () => {
      ...
      guard.recordProgress({ iteration: 1, modifiedFiles: 1, toolCallCount: 1, timestamp: Date.now() });
      guard.recordProgress({ iteration: 2, modifiedFiles: 1, toolCallCount: 2, timestamp: Date.now() });
      guard.recordProgress({ iteration: 3, modifiedFiles: 1, toolCallCount: 3, timestamp: Date.now() });
