...
describe('CheckpointManager', () => {
  ...
  describe('checkpoint creation', () => {
    ...
    it('should snapshot conversation state', async () => {
      ...
      state.addMessage({ role: 'assistant', content: 'Response 1', timestamp: 2000 });
    ...
    it('should snapshot memory state', async () => {
      ...
      expect(checkpoint?.memoryState.lastCheckpointId).toBe('previous-checkpoint');
    });

    it('should include operation journal', async () => {
      state.recordToolCall('ReadFile', { path: '/test.ts' }, { data: 'content' }, 100, true);
      state.recordToolCall('WriteFile', { path: '/out.ts' }, { success: true }, 200, true);
  ...
  describe('checkpoint restore', () => {
    it('should restore files from checkpoint', async () => {
      ...
      expect(fs.writeFile).toHaveBeenCalledWith('/src/test.ts', 'file content', 'utf-8');
    });
