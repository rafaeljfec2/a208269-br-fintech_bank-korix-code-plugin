...
- **Z**: Description Z`;
    ...
    const output = convertListsToTables(input);
    const tableCount = (output.match(/\| Name \| Description \|/g) ?? []).length;
...
describe('enhanceStructure', () => {
  it('converts file paths to links', () => {
    const input = 'Edit the file src/webview/components/chat/MarkdownContent.tsx to add the processor.';
    const output = enhanceStructure(input);
    expect(output).toContain('[src/webview/components/chat/MarkdownContent.tsx](src/webview/components/chat/MarkdownContent.tsx)');
  });
  ...
  it('supports various file extensions', () => {
    const input = 'Files: src/app.ts, src/style.css, src/config.json, src/test.test.tsx';
    const output = enhanceStructure(input);
...
Edit src/webview/utils/processor.ts to implement.`;
    ...
    // File path link
    expect(output).toContain('[src/webview/utils/processor.ts](src/webview/utils/processor.ts)');
  });
