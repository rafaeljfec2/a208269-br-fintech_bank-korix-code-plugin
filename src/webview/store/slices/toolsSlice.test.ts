...
describe('toolsSlice', () => {
  ...
  describe('removePendingApproval', () => {
    ...
    it('should only remove matching approval', () => {
      ...
      const state = store.getState();
      expect(state.pendingApprovals).toHaveLength(2);
      expect(state.pendingApprovals.find((a) => a.toolCallId === '1')).toBeDefined();
      expect(state.pendingApprovals.find((a) => a.toolCallId === '3')).toBeDefined();
      expect(state.pendingApprovals.find((a) => a.toolCallId === '2')).toBeUndefined();
    });
  ...
  describe('addToolCall', () => {
    ...
    it('should append multiple tool calls', () => {
      ...
      addToolCall({ id: '1', name: 'ReadFile', input: {}, status: 'pending', timestamp: 1000 });
      addToolCall({ id: '2', name: 'WriteFile', input: {}, status: 'success', timestamp: 2000 });
  ...
  describe('updateToolCall', () => {
    ...
    it('should not modify if id not found', () => {
      ...
      addToolCall({ id: 'call-1', name: 'ReadFile', input: {}, status: 'pending', timestamp: Date.now() });
  ...
  describe('clearToolCalls', () => {
    it('should remove all tool calls and pending approvals', () => {
      const { addToolCall, addPendingApproval, clearToolCalls } = store.getState();

      addToolCall({ id: '1', name: 'ReadFile', input: {}, status: 'completed' });
      addToolCall({ id: '2', name: 'WriteFile', input: {}, status: 'completed' });
      addPendingApproval({
